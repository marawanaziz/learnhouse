...
## Returning to an invitation

Existing learners who open a signup invitation can follow **Login** and then **Join Organization** to claim the linked course group. The invitation is retained through the login and forgot-password links, including the reset email. Auth screens use a local `returnTo` destination; external destinations are ignored. Existing checkout links using `redirect` remain supported.
