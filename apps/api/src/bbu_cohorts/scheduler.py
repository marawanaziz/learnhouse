"""In-process daily scheduler for cohort lifecycle bookkeeping.

Self-contained (no external cron / secret): a background task started at app
startup runs `service.run_lifecycle` once a day — flips started cohorts to
...
and multiple instances because the job is idempotent (a second run finds nothing
new to change).
"""
import asyncio
import logging
...
async def start_cohort_scheduler():
    """Startup hook — spawns the daily loop. Gated by BBU_COHORT_LIFECYCLE_ENABLED
    (default on)."""
