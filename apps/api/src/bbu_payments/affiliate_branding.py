...
def admin_page(settings, affs, totals, earned, base, admin_key=""):
    ...
    inner = (
        ...
        f"async function post(url,data){{data=data||{{}};data.key=KEY;const r=await fetch(url,{{method:'POST',"
        f"headers:{{'Content-Type':'application/json'}},body:JSON.stringify(data)}});return r.json()}}"
