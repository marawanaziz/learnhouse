...
- Android: a new RSA-2048 upload keystore was generated and its public PEM submitted through the existing app's **Request upload key reset** flow on September 9, 2026. Google currently shows the request as **pending**. Its notification confirms activation on **September 11, 2026 at 07:01 UTC (03:01 America/New_York)**; no new APK/AAB uploads are allowed until then. The new fingerprint is already displayed in the console, but it is not yet usable. New upload SHA-256: `8F:76:72:C1:08:55:44:3A:51:42:28:1D:49:DA:62:95:D3:0D:1C:5A:E6:70:8F:CE:47:AB:DD:FF:7B:64:67:B4`. Both passwords and private-key access were validated. The previous upload fingerprint remains pinned in `release/store-targets.json`, so the Android production preflight intentionally stops until Google's activation is verified. After activation, confirm the new fingerprint in Play Console, record that evidence, and update the pin. Google's actual app-signing key was not changed.
...
September 9 store preparation: Apple build 21 was validated, uploaded and processed successfully, with updated metadata, reviewer instructions and five branded screenshots in version 2.5.0. Google draft release 8 and updated sign-in instructions were saved; its production AAB is built and verified, awaiting upload-key activation. No Developer Program agreements, app capabilities, review submissions or store releases were changed. See `release/STATUS.md` for current evidence and remaining checks. The local Xcode software license was accepted as part of the authorized installation. The explicitly authorized signing credential setup on September 9 is documented above.
...
- Android preview APK compilation passed (330 Gradle tasks). APK v2 signature and the preview package, app name, minimum SDK 24 and target SDK 36 were verified. The artifact is `artifacts/android-preview.apk`; it has not undergone device acceptance.
- iOS archive and IPA export passed. The exported IPA passed deep/strict code-signature verification, certificate/profile matching, existing bundle/team checks and version 2.5.0/build 21 validation. Its embedded JavaScript bundle is present.
- The export subprocess prioritizes macOS tools in PATH: Apple’s rsync otherwise spawned an incompatible GNU rsync peer from Homebrew. The failure was reproduced in an isolated copy test, the corrected environment passed, and a fresh complete archive/export succeeded.
- The ordinary Android production preflight remains gated on Google upload-key activation. A separate protected preparation helper verified the confirmed pending key and built `artifacts/android-production-pending-key-activation.aab`; bundletool, package/version, signature and embedded JavaScript checks passed. It has not been uploaded.
- Apple authenticated IPA validation and upload succeeded; build 21 processed VALID and is attached to draft version 2.5.0 with manual release. Real course login and video playback passed in iPhone/iPad Release simulator previews. Remaining device checks are recorded in `release/STATUS.md`.
